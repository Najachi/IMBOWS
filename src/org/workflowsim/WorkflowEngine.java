...
//    	System.out.println(CloudSim.clock()+":"+CloudSim.getEntityName(ev.getSource())+"-> WorkflowEngine = "+ev.getTag());
    ...
    /**CODE AJOUTE POUR IMBO**/
    protected void processJobReturnForIMBO(SimEvent ev) throws Exception{
    	Job job = (Job) ev.getData();
        if (job.getCloudletStatus() == Cloudlet.FAILED) {
            // Reclustering engine will add retry job to jobList
        ...
        if(getJobsList().isEmpty() && jobsSubmitted == 0) {
    		...
    		if(index1 != IMBOScheduling.populationSize) {
    			already = 1;
             	FogBroker.count++;
    		...
    		else if(indexForUpdate != IMBOScheduling.populationSize){
    			int gbestIndex = 0;
    			for(int i = 0; i < index1; i++) {
    				if(fitness[i] < IMBOScheduling.gbest_fitness) {
    					IMBOScheduling.gbest_fitness = fitness[i];
    					 gbestIndex = i;
    				}
    			}
    			IMBOScheduling.gbest_schedule = IMBOScheduling.pbest_schedule.get(gbestIndex);
    			
    			FogBroker.count2++;
	        	fitness2[indexForUpdate++] = caculatefitness();
	        	if(FogBroker.count2 != IMBOScheduling.populationSize && IMBOScheduling.maxIterations > iterateNum) {
	        		init();
	        		sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
	        	}
 		    }
            if(index1 == IMBOScheduling.populationSize && indexForUpdate == IMBOScheduling.populationSize) {
            	if(IMBOScheduling.maxIterations > iterateNum) {
            		for(int i = 0; i < IMBOScheduling.populationSize; i++) { 
            			if(fitness[i] > fitness2[i]) {
            				int schedule1[] = IMBOScheduling.pbest_schedule.get(i);
            				int schedule2[] = IMBOScheduling.newSchedules.get(i);
            				for(int j = 0; j < schedule1.length; j++)
            					schedule1[j] = schedule2[j];
            				IMBOScheduling.pbest_fitness[i] = fitness2[i]; 
            			}
            			else
            				IMBOScheduling.pbest_fitness[i]=fitness[i];
	        		}
            		fitness = IMBOScheduling.pbest_fitness;
            		for(int i = 0; i < IMBOScheduling.populationSize; i++) {
            			if(IMBOScheduling.pbest_fitness[i] < IMBOScheduling.gbest_fitness) {
            				//index1=i;
            				IMBOScheduling.gbest_fitness = IMBOScheduling.pbest_fitness[i];
            				IMBOScheduling.gbest_schedule = IMBOScheduling.pbest_schedule.get(i);
            			}
            		}
            		iterateNum++;
            		System.out.println("After "+iterateNum+" iterations:");
            		System.out.println("======gbest_fitness:========"+IMBOScheduling.gbest_fitness);
            		updatebest.add(IMBOScheduling.gbest_fitness);
//	              	printindicators(PsoScheduling.gbest_fitness);
            		...
            		if(IMBOScheduling.maxIterations != iterateNum) {
        	        	...
        	        	init(); 
         		    	updateFlag2 = 1;
         		    	IMBOScheduling.newSchedules.removeAll(IMBOScheduling.newSchedules);
         		    	sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
    	        	}
            	}
            }
            ...
            if(IMBOScheduling.maxIterations == iterateNum && startlastSchedule == 0) {
            	for(int i = 0; i < IMBOScheduling.populationSize; i++) {
            		if(IMBOScheduling.pbest_fitness[i] == IMBOScheduling.gbest_fitness) {
            			IMBOScheduling.gbest_schedule = IMBOScheduling.pbest_schedule.get(i);
            		}
...
//    	sendNow(getcontrollerId(), FogEvents.STOP_SIMULATION,null);
    ...
    /**Calcul de la fonction fitness ou la Crowding distance de chaque solution sur le front de pareto dans IMBO**/
	// Méthode qui calcule la crowding distance de chaque solution sur le front de pareto
	// Le paramètre paretoFront est la liste des solutions pareto-optimales
	// Le résultat est un tableau des crowding distances de chaque solution
	
	public static double[] getCrowdingDistance(ArrayList<double[]> paretoFront) {
	    
		// Créer un tableau pour stocker les crowding distances
	    double[] crowdingDistance = new double[paretoFront.size()];
	    
	    // Initialiser les crowding distances à zéro
	    for (int i = 0; i < crowdingDistance.length; i++) {
	        crowdingDistance[i] = 0;
	    }
	    
	    // Parcourir les critères
	    for (int i_ = 0; i_ < paretoFront.get(0).length; i_++) {
	        
	    	//déclaration ajoutée car j'avais le message d'rreur "Local variable i defined in an enclosing scope must be final or effectively final"
	    	final int i = i_;
	    	
	    	// Trier les solutions selon le critère i
	        Collections.sort(paretoFront, new Comparator<double[]>() {
	            @Override
	            public int compare(double[] s1, double[] s2) {
	                return Double.compare(s1[i], s2[i]);
	            }
	        });
	        
	        // Attribuer une distance infinie aux solutions extrêmes
	        crowdingDistance[0] = Double.POSITIVE_INFINITY;
	        crowdingDistance[crowdingDistance.length - 1] = Double.POSITIVE_INFINITY;
	        
	        // Calculer la distance des solutions intermédiaires
	        for (int j = 1; j < crowdingDistance.length - 1; j++) {
	            
	        	// Ajouter la différence normalisée entre les solutions voisines
	            crowdingDistance[j] += (paretoFront.get(j + 1)[i] - paretoFront.get(j - 1)[i]) / (paretoFront.get(paretoFront.size() - 1)[i] - paretoFront.get(0)[i]);
	        }
	    }
	    
	    // Retourner le tableau des crowding distances
